...
import { submitQuoteRequest } from './action';
...
export default function QuoteRequestForm({
  ...
  const [state, action, isPending] = useActionState(submitQuoteRequest, undefined);
  ...
  return (
    ...
    >
      {/* Hidden fields for selected chips */}
      <input type="hidden" name="service" value={selectedService} />
      {selectedBudget && <input type="hidden" name="budget" value={selectedBudget} />}
      ...
      {state?.success && (
        <Typography className="text-green-600 font-medium bg-green-50 px-4 py-3 rounded-lg">
          ✓ Thank you for your {formType === 'quote' ? 'quote request' : 'request'}! We&apos;ll reach out to
          you soon.
        </Typography>
      )}
      {state?.error && (
